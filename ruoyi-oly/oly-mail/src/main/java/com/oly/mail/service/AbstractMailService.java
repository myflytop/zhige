...
import com.oly.mail.config.DefaultMailProperties;
...
public abstract class AbstractMailService implements IOlyMailService {
    ...
    protected synchronized DefaultMailProperties getMailProperties() {
        if (cachedMailProperties == null) {
            ...
            mailProperties.setPort(
                    Integer.parseInt(getMailMapConfig().get(EmailProperties.SSL_PORT.getValue()).getConfigValue()));
    ...
    private Map<String, SysConfig> getMailMapConfig() {
        if (this.sysConfigMap == null) {
            this.sysConfigMap = sysConfigService.selectConfigMap(EmailProperties.PREFIX.getValue());
        }
        return sysConfigMap;
    }
}