  ...
  if ($(".time-input").length > 0) {
    layui.use('laydate', function () {
      ...
      $(".time-input").each(function (index, item) {
        var time = $(item); // 控制控件外观

        var type = time.attr("data-type") || 'date'; // 控制回显格式

        var format = time.attr("data-format") || 'yyyy-MM-dd'; // 控制日期控件按钮

        var buttons = time.attr("data-btn") || 'clear|now|confirm',
            newBtnArr = []; // 日期控件选择完成后回调处理
...
var refreshItem = function refreshItem() {
...
var closeItem = function closeItem(dataId) {
  var topWindow = $(window.parent.document);

  if ($.common.isNotEmpty(dataId)) {
    window.parent.$.modal.closeLoading(); // 根据dataId关闭指定选项卡

    $('.menuTab[data-id="' + dataId + '"]', topWindow).remove(); // 移除相应tab对应的内容区
...
function createMenuItem(dataUrl, menuName, isRefresh) {
  ...
  var topWindow = $(window.parent.document); // 选项卡菜单已存在
  ...
  if (flag) {
    ...
    $('.menuTab', topWindow).removeClass('active'); // 添加选项卡对应的iframe
...
function scrollToTab(element) {
  ...
  var marginLeftVal = calSumWidth($(element).prevAll()),
      marginRightVal = calSumWidth($(element).nextAll()); // 可视区域非tab宽度

  var tabOuterWidth = calSumWidth($(".content-tabs", topWindow).children().not(".menuTabs")); //可视区域tab宽度

  var visibleWidth = $(".content-tabs", topWindow).outerWidth(true) - tabOuterWidth; //实际滚动宽度
...
var sub = {
  ...
  delRowByIndex: function delRowByIndex(value) {
    sub.editRow();
    $("#" + table.options.id).bootstrapTable('remove', {
      field: "index",
      values: [value]
  ...
  addRow: function addRow(row, tableId) {
...
function loadCss(file, headElem) {
  ...
  link.rel = 'stylesheet';
  link.type = 'text/css';
  if (headElem) headElem.appendChild(link);else document.getElementsByTagName('head')[0].appendChild(link);
} // 动态加载js文件
...
function loadJs(file, headElem) {
  ...
  if (headElem) headElem.appendChild(script);else document.getElementsByTagName('head')[0].appendChild(script);
} // 禁止后退键（Backspace）
...
window.onload = function () {
  document.getElementsByTagName("body")[0].onkeydown = function () {
    // 获取事件对象  
    var elem = event.relatedTarget || event.srcElement || event.target || event.currentTarget; // 判断按键为backSpace键  

    if (event.keyCode == 8) {
      // 判断是否需要阻止按下键盘的事件默认传递  
      var name = elem.nodeName;
      var className = elem.className; // 屏蔽特定的样式名称
