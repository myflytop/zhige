...
(function ($) {
  $.extend({
    ...
    table: {
      ...
      init: function init(options) {
        var defaults = {
          id: "bootstrap-table",
          type: 0,
          // 0 代表bootstrapTable 1代表bootstrapTreeTable
          method: 'post',
          height: undefined,
        ...
        $('#' + options.id).bootstrapTable({
          ...
          // 是否显示分页（*）
          paginationLoop: options.paginationLoop,
          // 是否启用分页条无限循环的功能
          pageNumber: 1,
          // 初始化加载第一页，默认第一页
          ...
          // 导出方式（默认all：导出所有数据；basic：导出当前页的数据；selected：导出选中的数据）
          exportTypes: options.exportTypes,
          // 导出文件类型 （json、xml、png、csv、txt、sql、doc、excel、xlsx、powerpoint、pdf）
          printPageBuilder: options.printPageBuilder,
          // 自定义打印页面模板
      ...
      getOptionsIds: function getOptionsIds(separator) {
      ...
      initEvent: function initEvent() {
        // 实例ID信息
        var optionsIds = $.table.getOptionsIds(); // 监听事件处理
        ...
        $(optionsIds).on("post-body.bs.table", function (e, args) {
          // 浮动提示框特效
          $(".table [data-toggle='tooltip']").tooltip(); // 气泡弹出框特效
        ...
        $(optionsIds).on("check.bs.table uncheck.bs.table check-all.bs.table uncheck-all.bs.table load-success.bs.table", function () {
          ...
          var uniqueId = table.options.uniqueId; // 工具栏按钮控制

          var rows = $.common.isEmpty(uniqueId) ? $.table.selectFirstColumns() : $.table.selectColumns(uniqueId); // 非多个禁用

          $('#' + toolbar + ' .multiple').toggleClass('disabled', !rows.length); // 非单个禁用
      ...
      // 表格销毁
      destroy: function destroy(tableId) {
      ...
      // 序列号生成
      serialNumber: function serialNumber(index, tableId) {
        var currentId = $.common.isEmpty(tableId) ? table.options.id : tableId;
      ...
      tooltip: function tooltip(value, length, target) {
      ...
      // 下拉按钮切换
      dropdownToggle: function dropdownToggle(value) {
        var actions = [];
      ...
      // 图片预览
      imageView: function imageView(value, height, width, target) {
      ...
      // 搜索-默认第一个form
      search: function search(formId, tableId, pageNumber, pageSize) {
        table.set(tableId);
      ...
      // 导出数据
      exportExcel: function exportExcel(formId) {
        table.set();
      ...
      // 导入数据
      importExcel: function importExcel(formId, width, height) {
        table.set();
      ...
      // 刷新表格
      refresh: function refresh(tableId, pageNumber, pageSize, url) {
        var currentId = $.common.isEmpty(tableId) ? table.options.id : tableId;
      ...
      // 刷新options配置
      refreshOptions: function refreshOptions(options, tableId) {
        var currentId = $.common.isEmpty(tableId) ? table.options.id : tableId;
        $("#" + currentId).bootstrapTable('refreshOptions', options);
      },
      // 查询表格指定列值 deDuplication（ true去重、false不去重）
      selectColumns: function selectColumns(column, deDuplication) {
      ...
      // 获取当前页选中或者取消的行ID
      affectedRowIds: function affectedRowIds(rows) {
      ...
      selectFirstColumns: function selectFirstColumns(deDuplication) {
        var distinct = $.common.isEmpty(deDuplication) ? true : deDuplication;
        var rows = $.map($("#" + table.options.id).bootstrapTable('getSelections'), function (row) {
      ...
      // 回显数据字典
      selectDictLabel: function selectDictLabel(datas, value) {
      ...
      // 回显数据字典（字符串数组）
      selectDictLabels: function selectDictLabels(datas, value, separator) {
        if ($.common.isEmpty(datas) || $.common.isEmpty(value)) {
          return '';
        }
      ...
      // 显示表格指定列
      showColumn: function showColumn(column, tableId) {
        var currentId = $.common.isEmpty(tableId) ? table.options.id : tableId;
        $("#" + currentId).bootstrapTable('showColumn', column);
      },
      // 隐藏表格指定列
      hideColumn: function hideColumn(column, tableId) {
        var currentId = $.common.isEmpty(tableId) ? table.options.id : tableId;
        $("#" + currentId).bootstrapTable('hideColumn', column);
      },
      // 显示所有表格列
      showAllColumns: function showAllColumns(tableId) {
        var currentId = $.common.isEmpty(tableId) ? table.options.id : tableId;
        $("#" + currentId).bootstrapTable('showAllColumns');
      },
      // 隐藏所有表格列
      hideAllColumns: function hideAllColumns(tableId) {
        var currentId = $.common.isEmpty(tableId) ? table.options.id : tableId;
    ...
    form: {
      // 表单重置
      reset: function reset(formId, tableId, pageNumber, pageSize) {
        table.set(tableId);
      ...
      // 获取选中复选框项
      selectCheckeds: function selectCheckeds(name) {
        var checkeds = "";
      ...
      // 获取选中下拉框项
      selectSelects: function selectSelects(name) {
        var selects = "";
    ...
    modal: {
      ...
      // 弹出提示
      alert: function alert(content, type) {
      ...
      // 消息提示，重新加载页面
      msgReload: function msgReload(msg, type) {
        top.layer.msg(msg, {
          icon: $.modal.icon(type),
          time: 500,
          shade: [0.1, '#8F8F8F']
      ...
      // 消息提示成功并刷新父窗体
      msgSuccessReload: function msgSuccessReload(msg) {
        $.modal.msgReload(msg, modal_status.SUCCESS);
      },
      // 获取iframe页的DOM
      getChildFrame: function getChildFrame(index) {
      ...
      // 弹出层指定宽度
      open: function open(title, url, width, height, callback) {
        // 如果是移动端，就使用自适应大小弹窗
        if ($.common.isMobile()) {
          width = 'auto';
          height = 'auto';
        ...
        if ($.common.isEmpty(callback)) {
          callback = function callback(index, layero) {
      ...
      openOptions: function openOptions(options) {
        ...
        var _btn = ['<i class="fa fa-check"></i> 确认', '<i class="fa fa-close"></i> 关闭']; // 如果是移动端，就使用自适应大小弹窗
      ...
      // 弹出层全屏
      openFull: function openFull(title, url, width, height) {
        // 如果是移动端，就使用自适应大小弹窗
        if ($.common.isMobile()) {
          width = 'auto';
          height = 'auto';
      ...
      // 选卡页方式打开
      openTab: function openTab(title, url, isRefresh) {
        createMenuItem(url, title, isRefresh);
      },
      // 选卡页同一页签打开
      parentTab: function parentTab(title, url) {
        var dataId = window.frameElement.getAttribute('data-id');
        createMenuItem(url, title);
        closeItem(dataId);
      },
      // 关闭选项卡
      closeTab: function closeTab(dataId) {
        closeItem(dataId);
      },
      // 禁用按钮
    ...
    operate: {
      // 提交数据
      submit: function submit(url, type, dataType, data, callback) {
        var config = {
          url: url,
          type: type,
          dataType: dataType,
      ...
      // 详细信息
      detail: function detail(id, width, height) {
        table.set();
      ...
      // 详细信息，以tab页展现
      detailTab: function detailTab(id) {
        table.set();
        $.modal.openTab("详细" + table.options.modalName, $.operate.detailUrl(id));
      },
      // 详细访问地址
      detailUrl: function detailUrl(id) {
        var url = "/404.html";
      ...
      // 修改信息，以tab页展现
      editTab: function editTab(id) {
        table.set();
        $.modal.openTab("修改" + table.options.modalName, $.operate.editUrl(id));
      },
      // 修改信息 全屏
      editFull: function editFull(id) {
        table.set();
        var url = "/404.html";
      ...
      // 保存信息 刷新表格
      save: function save(url, data, callback) {
        var config = {
          url: url,
          type: "post",
          dataType: "json",
      ...
      // 保存信息 弹出结果提示框
      saveModal: function saveModal(url, data, callback) {
        var config = {
          url: url,
          type: "post",
          dataType: "json",
      ...
      // 保存选项卡信息
      saveTab: function saveTab(url, data, callback) {
        var config = {
          url: url,
          type: "post",
          dataType: "json",
      ...
      // 保存结果弹出msg刷新table表格
      ajaxSuccess: function ajaxSuccess(result) {
      ...
      // 保存结果重新加载页面
      saveReload: function saveReload(result) {
        if (result.code == web_status.SUCCESS) {
      ...
      // 选项卡成功回调执行事件（父窗体静默更新）
      successTabCallback: function successTabCallback(result) {
        if (result.code == web_status.SUCCESS) {
    ...
    tree: {
      ...
      init: function init(options) {
        var defaults = {
          ...
          check: {
            enable: false,
            // 置 zTree 的节点上是否显示 checkbox / radio
            nocheckInherit: true,
            // 设置子节点是否自动继承
            chkboxType: {
              "Y": "ps",
        ...
        $.get(options.url, function (data) {
          ...
          $.tree.selectByIdName(treeId, node); // 回调tree方法
      ...
      searchNode: function searchNode() {
        ...
        var nodes = $._tree.getNodes(); // 如果要查空字串，就退出不查了。
        ...
        $.tree.hideAllNode(nodes); // 根据搜索值模糊匹配

        $.tree.updateNodes($._tree.getNodesByParamFuzzy("name", value));
      },
      // 根据Id和Name选中指定节点
      selectByIdName: function selectByIdName(treeId, node) {
        if ($.common.isNotEmpty(treeId) && node && treeId == node.id) {
          $._tree.selectNode(node, true);
        }
      },
      // 显示所有节点
      showAllNode: function showAllNode(nodes) {
      ...
      // 隐藏所有节点
      hideAllNode: function hideAllNode(nodes) {
      ...
      // 显示所有父节点
      showParent: function showParent(treeNode) {
        var parentNode;
      ...
      // 显示所有孩子节点
      showChildren: function showChildren(treeNode) {
      ...
      // 更新节点状态
      updateNodes: function updateNodes(nodeList) {
      ...
      // 获取当前被勾选集合
      getCheckedNodes: function getCheckedNodes(column) {
      ...
      // 不允许根父节点选择
      notAllowParents: function notAllowParents(_tree) {
      ...
      // 不允许最后层级节点选择
      notAllowLastLevel: function notAllowLastLevel(_tree) {
        var nodes = _tree.getSelectedNodes();
    ...
    common: {
      ...
      // 空对象转字符串
      nullToStr: function nullToStr(value) {
      ...
      // 比较两个字符串（大小写敏感）
      equals: function equals(str, that) {
        return str == that;
      },
      // 比较两个字符串（大小写不敏感）
      equalsIgnoreCase: function equalsIgnoreCase(str, that) {
        return String(str).toUpperCase() === String(that).toUpperCase();
      },
      // 将字符串按指定字符分割
      split: function split(str, sep, maxLen) {
      ...
      // 判断字符串是否是以start开头
      startWith: function startWith(value, start) {
        var reg = new RegExp("^" + start);
        return reg.test(value);
      },
      // 判断字符串是否是以end结尾
      endWith: function endWith(value, end) {
        var reg = new RegExp(end + "$");
        return reg.test(value);
      },
      // 数组去重
      uniqueFn: function uniqueFn(array) {
        var result = [];
        var hashObj = {};
      ...
      formToJSON: function formToJSON(formId) {
        var json = {};
      ...
      // 数据字典转下拉框
      dictToSelect: function dictToSelect(datas, value, name) {
        var actions = [];
      ...
      // 数字正则表达式，只能为0-9数字
      numValid: function numValid(text) {
        var patten = new RegExp(/^[0-9]+$/);
        return patten.test(text);
      },
      // 英文正则表达式，只能为a-z和A-Z字母
      enValid: function enValid(text) {
        var patten = new RegExp(/^[a-zA-Z]+$/);
        return patten.test(text);
      },
      // 英文、数字正则表达式，必须包含（字母，数字）
      enNumValid: function enNumValid(text) {
        var patten = new RegExp(/^(?=.*[a-zA-Z]+)(?=.*[0-9]+)[a-zA-Z0-9]+$/);
        return patten.test(text);
      },
      // 英文、数字、特殊字符正则表达式，必须包含（字母，数字，特殊字符!@#$%^&*()-=_+）
      charValid: function charValid(text) {
