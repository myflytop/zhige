...
<template>
	<view>
		<view class="zg-search">
			<u-search height="71" :show-action="true" v-model="keyword" action-text="搜索" :animation="true" @search="doSearch"
			...
			<scroll-view class="keyword-box" v-show="!isShowKeywordList" scroll-y>
				<view class="keyword-block" v-if="oldKeywordList.length>0">
					<view class="keyword-list-header">
						<view>历史搜索</view>
						<view>			
							<image @tap="deleteOne" v-if="deleteOneStatus" src="/static/HM-search/delete-one-pink.png"></image>
                            <image @tap="deleteOne" v-if="!deleteOneStatus" src="/static/HM-search/delete-one-blue.png"></image>
							<image @tap="oldDelete" src="/static/HM-search/delete.png"></image>
						</view>
					</view>
					<view class="keyword">
						<view v-for="(keyword,index) in oldKeywordList" :key="index" @tap="doSearchOld(keyword)"
							:class="{ 'se-one':  deleteOneStatus }">
							{{keyword}}
						</view>
					</view>
				</view>
...
<script>
	export default {
		...
		methods: {
			...
			deleteOne() {
				if (this.deleteOneStatus) {
					this.deleteOneStatus = false;
				} else {
					this.deleteOneStatus = true;
				}
			},
			//热门搜索开关
			hotToggle() {
			...
			doSearch(keyword) {
				...
				if (this.$u.trim(keyword).length > 0) {
				...
				uni.navigateTo({url:"/pages/tao/list/list?keyword="+keyword});
...
<style scoped>
   ...
   /deep/.u-action{
	...
	.keyword-box .keyword-block .keyword>.se-one {
		background-color: #033023;
		color: #FFFFFF;
	}
</style>
