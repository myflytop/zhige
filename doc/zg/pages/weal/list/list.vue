<template>
	<view class="">
		...
		<view class="zg-weal-swiper">
			<u-swiper :list="hotWealPosts"></u-swiper>
		</view>

		<view class="zg-weal-hot">
			<view class="item-container">
				<view class="thumb-box" v-for="(item1, index1) in wealCats" :key="index1">
		...
		<view class="zg-weal-list">
			...
			<uni-list>
				<!-- to属性：将新闻ID和标题传给详情页 -->
				<!-- TODO：需增加日期格式化的示例 -->
				<uni-list-item direction="row" v-for="item in wealPosts" :key="item.articleId"
					:title="item.articleTitle" :to="'/pages/weal/post/post?id='+item.articleId">
					<!-- 通过body插槽定义列表内容显示 -->
