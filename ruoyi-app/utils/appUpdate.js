...
// 弹窗图标url
const $iconUrl = "./static/images/app/ic_ar.png";
...
const cpr_version = function(a, b) {
	var _a = toNum(a),
		_b = toNum(b);
...
// 发起ajax请求获取服务端版本号
const getServerNo = function(version, isPrompt, callback) {
	request.get("appversion/getInfo", {
		equipmentType: (platform == "android") ? 1 : 2
	}).then(res => {
		if (res.versionNumber) {
			if (cpr_version(res.versionNumber, version.versionName)) {
				callback && callback(res);
				getApp().globalData.canlisten = false
			} else {
				getApp().globalData.canlisten = true
				getClipboard()
			}
		}
	});
...
export default function(isPrompt) {
	getCurrentNo(versionInfo => {
		getServerNo(versionInfo, isPrompt, res => {
			if (res.method == 1) {
				if (/\.wgt$/i.test(res.downUrl)) {
					getDownload(res);
				} else if (/\.html$/i.test(res.downUrl)) {
					plus.runtime.openURL(res.downUrl);
				} else {
					if (platform == "android") {
						getDownload(res);
					} else {
