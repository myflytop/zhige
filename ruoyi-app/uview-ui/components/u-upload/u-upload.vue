<template>
	<view class="u-upload" v-if="!disabled">
		...
		>
			...
			<image @tap.stop="doPreviewImage(item.url || item.path, index)" class="u-preview-image" v-if="$jsutl.isImageOrVideo(item.url || item.path) == 0" :src="item.url || item.path" :mode="imageMode"></image>
			<video :src="item.url || item.path" class="u-preview-image" :controls="false"  v-if="$jsutl.isImageOrVideo(item.url || item.path) == 1">
				<cover-image class="controls-delete img" @click="deleteItem(index)" src="/static/images/close.png"></cover-image>
			</video>
		</view>
