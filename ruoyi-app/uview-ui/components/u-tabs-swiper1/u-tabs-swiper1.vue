			...
			<view class="u-tabs-scroll-box" :class="{'u-tabs-scorll-flex': !isScroll}">
				<view class="u-tabs-item" :style="[tabItemStyle(index)]"
				 v-for="(item, index) in getTabs" :key="index"  :class="[preId + index]" @tap="emit(index)">
					<image :src="item.bgImg" style="width: 120rpx;height: 90rpx;position: relative;z-index: -1;"></image>
					<view style="font-size: 26rpx;color: #FFFFFF;position: absolute;z-index: 99;bottom: 16rpx;left: 50%; transform: translate(-50%);line-height: 26rpx;">
						<view :style="current==index?'border-bottom: 4rpx solid #FFFFFF;border-radius:4rpx;padding-bottom:6rpx;':'padding-bottom:6rpx;'" style="position: relative;">
							{{ item[name] || item['name']}}
						</view>
					</view>
				</view>
